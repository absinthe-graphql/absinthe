defmodule AbsintheTest do
  ...
  test "can return multiple errors" do
    ...
    assert_result {:ok, %{data: %{"failingThing" => nil}, errors: [%{message: "one", path: ["failingThing"]}, %{message: "two", path: ["failingThing"]}]}}, run(query, Absinthe.Fixtures.ThingsSchema)
  end

  test "can return extra error fields" do
    query = "mutation { failingThing(type: WITH_CODE) { name } }"
    assert_result {:ok, %{data: %{"failingThing" => nil}, errors: [%{code: 42, message: "Custom Error", path: ["failingThing"]}]}}, run(query, Absinthe.Fixtures.ThingsSchema)
  end

  test "requires message in extended errors" do
    query = "mutation { FailingThing(type: WITHOUT_MESSAGE) { name } }"
    assert_raise Absinthe.ExecutionError, fn -> run(query, Absinthe.Fixtures.ThingsSchema) end
  end

  test "can return multiple errors, with extra error fields" do
    query = "mutation { failingThing(type: MULTIPLE_WITH_CODE) { name } }"
    assert_result {:ok, %{data: %{"failingThing" => nil}, errors: [%{code: 1, message: "Custom Error 1", path: ["failingThing"]}, %{code: 2, message: "Custom Error 2", path: ["failingThing"]}]}}, run(query, Absinthe.Fixtures.ThingsSchema)
  end
  ...
  test "can do a simple query" do
    ...
    """
    assert_result {:ok, %{data: %{"thing" => %{"name" => "Foo"}}}}, run(query, Absinthe.Fixtures.ThingsSchema)
  end
  ...
  test "can do a simple query with a weird alias" do
    ...
    """
    assert_result {:ok, %{data: %{"thing" => %{"fOO_Bar_baz" => "Foo"}}}}, run(query, Absinthe.Fixtures.ThingsSchema)
  end
  ...
  test "can do a simple query returning a list" do
    ...
    """
    assert_result {:ok, %{data: %{"things" => [%{"name" => "Bar", "id" => "bar"}, %{"name" => "Foo", "id" => "foo"}]}}}, run(query, Absinthe.Fixtures.ThingsSchema)
  end
  ...
  test "returns an error message when a list is given where it doesn't belong" do
    ...
    assert_result {:ok, %{errors: [%{locations: [%{column: 0, line: 2}],
      message: "Argument \"id\" has invalid value [\"foo\"]."}]}}, run(query, Absinthe.Fixtures.ThingsSchema)
  end
  ...
  test "Invalid arguments on children of a list field are correctly handled" do
    ...
    """
    assert_result {:ok, %{errors: [%{message: "Unknown argument \"x\" on field \"id\" of type \"Thing\"."}]}}, run(query, Absinthe.Fixtures.ThingsSchema)
  end
  ...
  test "can do a simple query with an all caps alias" do
    ...
    """
    assert_result {:ok, %{data: %{"thing" => %{"FOO" => "Foo"}}}}, run(query, Absinthe.Fixtures.ThingsSchema)
  end
  ...
  test "can identify a bad field" do
    ...
    """
    assert_result {:ok, %{errors: [%{message: ~s(Cannot query field "bad" on type "Thing".)}]}}, run(query, Absinthe.Fixtures.ThingsSchema)
  end
  ...
  test "returns the correct results for an alias" do
    ...
    """
    assert_result {:ok, %{data: %{"widget" => %{"name" => "Foo"}}}}, run(query, Absinthe.Fixtures.ThingsSchema)
  end

  test "checks for required arguments" do
    query = "{ thing { name } }"
    assert_result {:ok, %{errors: [%{message: ~s(In argument "id": Expected type "String!", found null.)}]}}, run(query, Absinthe.Fixtures.ThingsSchema)
  ...
  test "checks for extra arguments" do
    ...
    """
    assert_result {:ok, %{errors: [%{message: ~s(Unknown argument "extra" on field "thing" of type "RootQueryType".)}]}}, run(query, Absinthe.Fixtures.ThingsSchema)
  end
  ...
  test "checks for badly formed arguments" do
    ...
    """
    assert_result {:ok, %{errors: [%{message: ~s(Argument "val" has invalid value "AAA".)}]}}, run(query, Absinthe.Fixtures.ThingsSchema)
  end
  ...
  test "returns nested objects" do
    ...
    """
    assert_result {:ok, %{data: %{"thing" => %{"name" => "Foo", "otherThing" => %{"name" => "Bar"}}}}}, run(query, Absinthe.Fixtures.ThingsSchema)
  end
