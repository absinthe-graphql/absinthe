defmodule Absinthe.ValidationPhaseCase do
  ...
  defmacro __using__(opts) do
    ...
    quote do
        ...
        fn
          pairs ->
            ...
            matched = Enum.any?(pairs, fn
              {%str{} = node, %Phase.Error{phase: unquote(phase), message: ^message} = err} when str == node_kind ->
      ...
      @spec assert_fails_validation(Language.Source.t, map, [Absinthe.ValidationPhaseCase.error_checker_t] | Absinthe.ValidationPhaseCase.error_checker_t) :: no_return
      def assert_fails_validation(document, options, error_checker) do
        assert_invalid(Absinthe.Fixtures.PetsSchema, [unquote(phase)], document, options, error_checker)
      end

      @spec assert_passes_validation_with_schema(Schema.t, Language.Source.t, map) :: no_return
      def assert_passes_validation_with_schema(schema, document, options) do
        assert_valid(schema, [unquote(phase)], document, options)
      end

      @spec assert_fails_validation_with_schema(Schema.t, Language.Source.t, map, Absinthe.ValidationPhaseCase.error_checker_t) :: no_return
  ...
  def assert_valid(schema, validations, document, options) do
    result = case run(schema, validations, document, options) do
      {:ok, result} ->
        result
      # :jump, etc
