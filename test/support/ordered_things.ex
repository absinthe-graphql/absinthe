defmodule OrderedThings do
  use OrdMap
  use Absinthe.Schema

  @db o%{
    "foo" => o(%{id: "foo", name: "Foo", value: 4}),
    "bar" => o%{id: "bar", name: "Bar", value: 5}
  }
    ...
    field :update_thing,
      ...
      resolve: fn
        %{id: id, thing: %{value: val}}, _ ->
          found = @db |> OrdMap.get(id)
          {:ok, OrdMap.replace(found, :value, val)}
        %{id: id, thing: fields}, _ ->
          found = @db |> OrdMap.get(id)
          {:ok, found |> OrdMap.merge(fields)}
      end
  ...
  query do
    ...
    field :things, list_of(:thing) do
      resolve fn _, _ ->
        {:ok, @db |> OrdMap.values |> Enum.sort_by(&OrdMap.get(&1,:id))}
      end
    end
  ...
  object :thing do
    ...
    field :fail, :id do
      ...
      resolve fn
        data, %{id: id}, _ ->
          if OrdMap.get(data, :id) == id,
            do: {:error, "fail"},
            else: {:ok, OrdMap.get(data, :id)}
      end
    end
    ...
    field :other_thing,
      type: :thing,
      resolve: fn (_, %{source: sourceData}) ->
        case OrdMap.get(sourceData, :id) do
          "foo" -> {:ok, @db |> OrdMap.get("bar")}
          "bar" -> {:ok, @db |> OrdMap.get("foo")}
        end
      end
