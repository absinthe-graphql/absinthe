defmodule Absinthe.Execution.OrderedSubscriptionTest do
  use Absinthe.Case, async: false, ordered: true, import_run: false
  use OrdMap
  ...
  test "different subscription docs are batched together" do
    ...
    assert_receive({:broadcast, %{topic: ^doc2, result: result}})

    name = result.data 
        |> OrdMap.get("user")
        |> OrdMap.get("group")
        |> OrdMap.get("name")
    assert name == "Elixir Users"
