defmodule Absinthe.OrderedUnionFragmentTest do
  ...
  test "it queries a heterogeneous list properly" do
    ...
    """
    expected = o%{"viewer" => o%{"objects" => [
      o(%{"__typename" => "User", "name" => "foo"}),
      o(%{"__typename" => "Todo", "completed" => false}),
      o%{"__typename" => "User", "name" => "bar"}
    ]}}
  ...
  test "it queries an interface with the concrete type's field resolvers" do
    ...
    """
    expected = o%{"viewer" => o%{"me" => o%{"__typename" => "User", "name" => "baz"}}}
    assert {:ok, %{data: expected}} == Absinthe.run(doc, Schema)
  end
  ...
  test "it queries an interface implemented by a union type" do
    ...
    expected = o%{"viewer" => o%{"objects" => [
      o(%{"__typename" => "User", "name" => "foo"}),
      o(%{"__typename" => "Todo", "name" => "do stuff"}),
      o%{"__typename" => "User", "name" => "bar"}
    ]}}
    assert {:ok, %{data: expected}} == Absinthe.run(doc, Schema)
  end
  ...
  test "it queries an interface on an unrelated interface" do
    ...
    """
    expected = o%{"viewer" => o%{"namedThing" =>
      o%{"__typename" => "Todo", "name" => "do stuff", "completed" => false}
    }}
    assert {:ok, %{data: expected}} == Absinthe.run(doc, Schema)
  end
