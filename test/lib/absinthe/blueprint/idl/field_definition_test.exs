defmodule Absinthe.Blueprint.IDL.FieldDefinitionTest do
  ...
  describe ".from_ast" do
    ...
    it "works, given an IDL object field definition" do
      ...
      field_def = fields |> List.first |> Blueprint.IDL.FieldDefinition.from_ast(doc)
    ...
    it "captures directives" do
      {doc, fields} = fields_from_input(@idl)
      field_def = fields |> Enum.at(1) |> Blueprint.IDL.FieldDefinition.from_ast(doc)
      assert %Blueprint.IDL.FieldDefinition{name: "baz"} = field_def
    end

    it "includes argument definitions" do
      {doc, fields} = fields_from_input(@idl)
      field_def = fields |> Enum.at(2) |> Blueprint.IDL.FieldDefinition.from_ast(doc)
      assert %Blueprint.IDL.FieldDefinition{name: "quuxes", type: %Blueprint.ListType{of_type: %Blueprint.NamedType{name: "Quux"}}, arguments: [%Blueprint.IDL.ArgumentDefinition{name: "limit", type: %Blueprint.NamedType{name: "Int"}, default_value: %Blueprint.Input.Integer{value: 4}}]} = field_def
    end
