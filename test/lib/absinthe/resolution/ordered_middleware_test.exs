defmodule Absinthe.OrderedMiddlewareTest do
  ...
  test "email works when current user" do
    ...
    assert o(%{"public" => o(%{"name" => "bob", "email" => "secret"})}) == data
  end
  ...
  test "secret object cant be accessed without a current user" do
    ...
    {returnsPrivateObject { key }}
    """
    assert {:ok, %{data: o%{"returnsPrivateObject" => o%{"key" => "value"}}}} == Absinthe.run(doc, __MODULE__.Schema, context: %{current_user: %{}})
  end
  ...
  test "it can modify the context" do
    ...
    """
    assert {:ok, %{data: data}} = Absinthe.run(doc, __MODULE__.Schema, context: %{current_user: %{}})
    assert o(%{"fromContext" => "yooooo"}) == data
  end
  ...
  test "it gets the path of the current field" do
    ...
    """
    assert {:ok, %{data: data}} = Absinthe.run(doc, __MODULE__.Schema, context: %{current_user: %{}})
    assert o(%{"foo" => o(%{"bar" => o(%{"result" => ["result", "bar", "foo", "RootQueryType"]})})}) == data
  end
end
