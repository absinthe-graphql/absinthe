defmodule Absinthe.Execution.Arguments.BooleanTest do
  ...
  test "variables are passed as arguments to resolution functions correctly" do
    assert_data %{"something" => "YES"},
      run(@graphql, @schema,
        variables: %{"flag" => true})

    assert_data %{"something" => "NO"},
      run(@graphql, @schema,
  ...
  test "literals are passed as arguments to resolution functions correctly" do
    assert_data %{"something" => "YES"},
      run(~s<query { something(flag: true) }>, @schema)

    assert_data %{"something" => "NO"},
      run(~s<query { something(flag: false) }>, @schema)

    assert_data %{"something" => "NO"},
      run(~s<query { something }>, @schema)
  end
