defmodule Absinthe.IntegrationTest do
  ...
  def assert_scenario(%{name: "execution/introspection/schema_types"} = definition, {options, _expectation}) do
    result = run(definition.graphql, definition.schema, options)
  ...
  def assert_scenario(%{name: "execution/introspection/full"} = definition, {options, _expectation}) do
    result = run(definition.graphql, definition.schema, options)
