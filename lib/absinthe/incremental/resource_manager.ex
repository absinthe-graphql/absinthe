defmodule Absinthe.Incremental.ResourceManager do
  ...
  @default_config %{
    ...
    max_memory_mb: 500,
    check_interval: 5_000  # Check resources every 5 seconds
  }
  ...
  def handle_cast({:release, operation_id}, state) do
    case Map.get(state.active_streams, operation_id) do
      ...
      stream_info ->
        ...
        Logger.debug("Released stream slot for operation #{operation_id} (duration: #{duration}ms)")
  ...
  defp check_stale_streams(state) do
    ...
    stale_streams = 
      ...
      |> Enum.filter(fn {_id, info} ->
        (now - info.started_at) > max_duration * 2  # 2x timeout = definitely stale
      end)
