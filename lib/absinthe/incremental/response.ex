defmodule Absinthe.Incremental.Response do
  ...
  """
  @spec build_stream_incremental(list(), list(), String.t() | nil, boolean()) :: incremental_response()
  ...
  def build_completed(completed_ids) do
    completed_items = Enum.map(completed_ids, fn id ->
      %{id: id}
    end)
  ...
  defp build_pending_list(streaming_context) do
    deferred = Enum.map(streaming_context.deferred_fragments || [], fn fragment ->
      pending = %{
    ...
    streamed = Enum.map(streaming_context.streamed_fields || [], fn field ->
      pending = %{
        id: generate_pending_id(),
        path: field.path
      }
