defmodule Absinthe.Type.Scalar do
  ...
  The following built-in scalar types are defined:
    ...
    * `:integer` - Represents non-fractional signed whole numeric value. **By default it is not** compliant with the GraphQl Specification**, it can represent values between `-(2^53 - 1)` and `2^53 - 1` as specified by [IEEE 754](http://en.wikipedia.org/wiki/IEEE_floating_point). To use the [GraphQl compliant Int](https://spec.graphql.org/October2021/#sec-Int), representing values between `-2^31` and `2^31 - 1`, set `config :absinthe, use_legacy_non_compliant_integers: :false` in your config. Note that Absinthe uses the full word `:integer` to identify this type, but its `name` (used by variables, for instance), is `"Int"`.
