defmodule Absinthe.Schema.Verification.Unions do
  ...
  def check(schema) do
    ...
    |> Enum.reduce(schema, fn
      %{types: concrete_types} = union, acc ->
    ...
    case schema.types[concrete_type_ident] do
      nil ->
        ...
        check_resolvers(
          union,
          rest,
          %{schema | errors: [err | schema.errors]}
        )
      %{is_type_of: nil} ->
        err = "Union type :#{ident} does not provide a `resolve_type` function and concrete type :#{concrete_type_ident} does not provide an `is_type_of` function. There is no way to resolve this concrete type during execution."
        check_resolvers(
          union,
          rest,
          %{schema | errors: [err | schema.errors]}
