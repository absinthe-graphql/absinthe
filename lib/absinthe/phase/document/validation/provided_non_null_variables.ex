defmodule Absinthe.Phase.Document.Validation.ProvidedNonNullVariables do
  ...
  defp handle_node(%Blueprint.Document.VariableDefinition{type: %Blueprint.TypeReference.NonNull{}, provided_value: nil} = node, _) do
    node
    |> put_error(error(node))
  end
  defp handle_node(%Blueprint.Document.VariableDefinition{type: %Blueprint.TypeReference.NonNull{}, provided_value: %Blueprint.Input.Null{}} = node, _) do
    node
    |> put_error(error(node))
  end
